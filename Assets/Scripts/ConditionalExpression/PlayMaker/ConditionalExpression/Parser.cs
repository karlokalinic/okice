...
namespace PlayMaker.ConditionalExpression
{
	public sealed class Parser
	{
		...
		private Node compare_expr_rest(Node exp)
		{
			...
			{
				Tokenizer.Next();
				TokenType op = 0 + (token.type - 4);
				Node right = expr(Tokenizer.Next());
				exp = new LogicalCompareNode((ComparisonOperator)op, exp, right);
				if (Tokenizer.HasFinished)
				{
					return exp;
				}
		...
		private Node expr_rest(Node term)
		{
			...
			{
				Tokenizer.Next();
				bool op = !(token.content == "+");
				Node right = expr_term(Tokenizer.Next());
				term = new BinaryExpressionNode(op ? BinaryOperator.Subtract : BinaryOperator.Add, term, right);
				if (Tokenizer.HasFinished)
				{
					return term;
				}
		...
		private Node expr_term_rest(Node term)
		{
			...
			{
				Tokenizer.Next();
				int op = ((token.content == "*") ? 2 : ((token.content == "/") ? 3 : 4));
				Node right = expr_value(Tokenizer.Next());
				term = new BinaryExpressionNode((BinaryOperator)op, term, right);
				if (Tokenizer.HasFinished)
				{
					return term;
				}
		...
		private Node expr_value(Token token)
		{
			...
			case TokenType.Symbol:
				if (token.content == "(")
				{
					...
					{
						throw new SyntaxErrorException($"Found '{token.content}' but expected ')'.");
					}
			...
			throw new SyntaxErrorException($"Expected value but found '{token.content}'.");
		}
