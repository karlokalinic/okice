...
namespace HutongGames.PlayMaker
{
	public class FsmLog
	{
		...
		public List<FsmLogEntry> Entries => entries;
		...
		private void AddEntry(FsmLogEntry entry, bool sendToUnityLog = false)
		{
			...
			case FsmLogType.Error:
				Debug.LogError(FormatUnityLogString(entry.Text), entry.Fsm.OwnerObject);
				return;
			case FsmLogType.Warning:
				Debug.LogWarning(FormatUnityLogString(entry.Text), entry.Fsm.OwnerObject);
				return;
			}
			if ((MirrorDebugLog || sendToUnityLog) && entry.LogType != FsmLogType.Transition)
			{
				Debug.Log(FormatUnityLogString(entry.Text), entry.Fsm.OwnerObject);
			}
		}
		...
		public void LogExitState(FsmState state)
		{
			if (state != null)
			{
				...
				if (EnableDebugFlow && state.Fsm != null && state.Fsm.EnableDebugFlow && !PlayMakerFSM.ApplicationIsQuitting)
				{
		...
		public void LogEnterState(FsmState state)
		{
			if (state != null)
			{
				...
				fsmLogEntry.State = state;
				if (EnableDebugFlow && state.Fsm != null && state.Fsm.EnableDebugFlow)
				{
					fsmLogEntry.FsmVariablesCopy = new FsmVariables(state.Fsm.Variables);
					fsmLogEntry.GlobalVariablesCopy = new FsmVariables(FsmVariables.GlobalVariables);
				}
