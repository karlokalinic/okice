...
namespace HutongGames.PlayMaker
{
	public static class ReflectionUtils
	{
		...
		public static Type GetGlobalType(string typeName)
		{
			...
			typeLookup.TryGetValue(typeName, out var value);
			...
			if ((object)value == null)
			{
				...
				if ((object)value == null)
				{
					for (int j = 0; j < loadedAssemblies.Length; j++)
					{
						...
						{
							Debug.Log("Failed to GetTypes: " + loadedAssemblies[j].FullName + "\n" + ex.Message);
						}
					}
				}
			}
		...
		public static void SetMemberValue(MemberInfo[] memberInfo, object target, object value)
		{
			...
			{
				SetBoxedMemberValue(parent, targetInfo, target, memberInfo[^1], value);
			}
			else
			{
				SetMemberValue(memberInfo[^1], target, value);
			}
		}
