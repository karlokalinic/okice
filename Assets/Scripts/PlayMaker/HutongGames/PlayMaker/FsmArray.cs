...
namespace HutongGames.PlayMaker
{
	...
	public class FsmArray : NamedVariable
	{
		...
		public string ObjectTypeName => objectTypeName;
		...
		public override VariableType TypeConstraint => type;
		...
		private void InitObjectType()
		{
			...
			{
				if (ElementType == VariableType.GameObject)
				{
					objectTypeName = typeof(GameObject).FullName;
				}
				else if (ElementType == VariableType.Material)
				{
					objectTypeName = typeof(Material).FullName;
				}
				else if (ElementType == VariableType.Texture)
				{
					objectTypeName = typeof(Texture).FullName;
				}
				else if (ElementType == VariableType.Enum)
				{
		...
		public void Resize(int newLength)
		{
			if (newLength != Values.Length)
			{
				...
				Array array = Array.CreateInstance(Values.GetType().GetElementType(), newLength);
		...
		{
			Array array = Array.CreateInstance(Values.GetType().GetElementType(), values.Length + 1);
			if (atIndex > values.Length)
			{
				atIndex = values.Length;
			}
			if (atIndex > 0)
			{
				Array.Copy(values, 0, array, 0, atIndex);
			}
			if (atIndex < values.Length)
			{
				Array.Copy(values, atIndex, array, atIndex + 1, array.Length - atIndex - 1);
			}
			array.SetValue(value, atIndex);
			Values = (object[])array;
			SaveChanges();
		}
		...
		public override string ToString()
		{
			...
			for (int i = 0; i < Values.Length; i++)
			{
				...
				else
				{
					...
					text = ((!(obj2 != null)) ? (text + obj.ToString()) : (text + obj2.name));
				}
		...
		public override bool TestTypeConstraint(VariableType variableType, Type _objectType = null)
		{
			...
			if (base.TestTypeConstraint(variableType, objectType))
			{
				...
				if ((object)ObjectType != _objectType)
				{
					return (object)_objectType == null;
				}
				return true;
			}
			return false;
