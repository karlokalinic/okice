...
namespace HutongGames.PlayMaker
{
	public class FsmLogEntry
	{
			...
			get
			{
				if (text == null)
				{
					...
					case FsmLogType.ExitState:
						text = $"EXIT: {State.Name} [{StateTime:f2}s]";
						break;
		...
		public string TextWithTimecode => textWithTimecode ?? (textWithTimecode = FsmTime.FormatTime(Time) + " " + Text);
