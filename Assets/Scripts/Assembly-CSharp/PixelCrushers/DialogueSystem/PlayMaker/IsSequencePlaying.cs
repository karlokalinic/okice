...
namespace PixelCrushers.DialogueSystem.PlayMaker
{
	...
	public class IsSequencePlaying : FsmStateAction
	{
		...
		{
			Sequencer obj = sequencerHandle.Value as Sequencer;
			bool isPlaying = obj.IsPlaying;
			if (obj != null && everyFrame != null)
			{
				everyFrame.Value = isPlaying;
			}
			if (isPlaying)
			{
				base.Fsm.Event(playingEvent);
			}
			else
			{
				base.Fsm.Event(notPlayingEvent);
			}
		}
	}
}
