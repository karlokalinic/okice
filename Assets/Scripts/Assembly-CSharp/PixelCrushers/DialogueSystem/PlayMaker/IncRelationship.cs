...
namespace PixelCrushers.DialogueSystem.PlayMaker
{
	...
	public class IncRelationship : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			{
				Lua.Run($"IncRelationship(Actor[\"{DialogueLua.StringToTableIndex(actor1Name.Value)}\"], Actor[\"{DialogueLua.StringToTableIndex(actor2Name.Value)}\"], \"{relationshipType.Value}\", {incrementAmount.Value})", DialogueDebug.LogInfo);
			}
			Finish();
		}
	}
