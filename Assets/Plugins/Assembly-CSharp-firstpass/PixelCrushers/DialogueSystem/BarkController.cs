...
namespace PixelCrushers.DialogueSystem
{
	public static class BarkController
	{
		...
		public static IEnumerator Bark(string conversationTitle, Transform speaker, Transform listener, BarkHistory barkHistory, DialogueDatabase database = null, bool stopAtFirstValid = false)
		{
			...
			else if (DialogueManager.useLinearGroupMode)
			{
				conversationModel.UpdateResponses(firstState);
			}
			...
			try
			{
				...
				barked = true;
				InformParticipants("OnBarkStart", "OnBarkStartSpeaker", speaker, listener);
			...
			finally
			{
				if (barked)
				{
					InformParticipants("OnBarkEnd", "OnBarkEndSpeaker", speaker, listener);
		...
		private static Sequencer PlayBarkSequence(string barkText, string sequence, string entrytag, Transform speaker, Transform listener)
		{
			...
			if (!string.IsNullOrEmpty(sequence))
			{
				...
				if (sequence.Contains("{{end}}"))
				{
					...
					sequence = sequence.Replace("{{end}}", ((DialogueManager.displaySettings.barkSettings.barkCharsPerSecond > 0f) ? Mathf.Max(DialogueManager.displaySettings.barkSettings.minBarkSeconds, (float)num / Mathf.Max(1f, DialogueManager.displaySettings.barkSettings.barkCharsPerSecond)) : Mathf.Max(DialogueManager.displaySettings.GetMinSubtitleSeconds(), (float)num / Mathf.Max(1f, DialogueManager.displaySettings.GetSubtitleCharsPerSecond()))).ToString(CultureInfo.InvariantCulture));
				}
		...
		private static void InformParticipants(string message, string messageSpeaker, Transform speaker, Transform listener)
		{
			if (speaker != null)
			{
				Transform parameter = ((listener != null) ? listener : speaker);
				speaker.BroadcastMessage(message, parameter, SendMessageOptions.DontRequireReceiver);
			...
			{
				Transform parameter2 = ((speaker != null) ? speaker : ((listener != null) ? listener : transform));
