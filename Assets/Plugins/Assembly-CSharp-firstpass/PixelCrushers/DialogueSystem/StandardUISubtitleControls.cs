...
namespace PixelCrushers.DialogueSystem
{
	...
	public class StandardUISubtitleControls : AbstractUISubtitleControls
	{
		...
		public void Initialize(StandardUISubtitlePanel[] subtitlePanels, StandardUISubtitlePanel defaultNPCSubtitlePanel, StandardUISubtitlePanel defaultPCSubtitlePanel, StandardDialogueUI dialogueUI)
		{
		...
		{
			this.dialogueUI = dialogueUI;
			m_builtinPanels.ForEach(delegate(StandardUISubtitlePanel panel)
			{
		...
		public void OverrideActorPanel(Actor actor, SubtitlePanelNumber subtitlePanelNumber, StandardUISubtitlePanel customPanel = null, bool immediate = false)
		{
			...
			if (standardUISubtitlePanel != panelFromNumber && standardUISubtitlePanel != null && (standardUISubtitlePanel.portraitImage == null || standardUISubtitlePanel.portraitImage.sprite == actor.GetPortraitSprite()))
			{
				...
				{
					panelFromNumber.Open(dialogueUI);
		...
		public StandardUISubtitlePanel GetActorTransformPanel(Transform speakerTransform, StandardUISubtitlePanel defaultPanel, out DialogueActor dialogueActor)
		{
			...
			standardUISubtitlePanel.dialogueUI = dialogueUI;
		...
		public void ShowActorInPanel(Actor actor, SubtitlePanelNumber subtitlePanelNumber, StandardUISubtitlePanel customPanel = null)
		{
			...
			string text = DialogueLua.GetLocalizedActorField(name, "Display Name").asString;
			if (string.IsNullOrEmpty(text))
			{
				text = actor.localizedName;
			}
			panelFromNumber.OpenOnStartConversation(actor.GetPortraitSprite(), name, text, null, dialogueUI);
		}
		...
		public virtual void RecordActorPanelCache(out List<string> actorGOs, out List<SubtitlePanelNumber> actorGOPanels, out List<int> actorIDs, out List<SubtitlePanelNumber> actorIDPanels, out List<string> actorNames)
		{
			...
			foreach (KeyValuePair<int, StandardUISubtitlePanel> item in m_actorIdOverridePanel)
			{
				actorIDs.Add(item.Key);
				StandardUISubtitlePanel value = item.Value;
				SubtitlePanelNumber subtitlePanelNumberFromPanel = GetSubtitlePanelNumberFromPanel(item.Value, allowReturnDefault: false);
				actorIDPanels.Add(subtitlePanelNumberFromPanel);
				if (subtitlePanelNumberFromPanel < SubtitlePanelNumber.Panel0)
				{
					continue;
				}
				Actor actor = DialogueManager.masterDatabase.GetActor(item.Key);
				if (actor != null)
				{
					int num = (int)(subtitlePanelNumberFromPanel - 3);
					if (0 <= num && num < m_builtinPanels.Count && actor.Name == value.portraitActorName)
					{
						actorNames[num] = actor.Name;
					}
				}
			}
			foreach (KeyValuePair<Transform, StandardUISubtitlePanel> item2 in m_actorPanelCache)
			{
				if (item2.Key == null)
				{
					continue;
				}
				StandardUISubtitlePanel value2 = item2.Value;
				SubtitlePanelNumber subtitlePanelNumberFromPanel2 = GetSubtitlePanelNumberFromPanel(item2.Value, allowReturnDefault: false);
				if (subtitlePanelNumberFromPanel2 == SubtitlePanelNumber.Custom)
				{
					continue;
				}
				actorGOs.Add(item2.Key.name);
				actorGOPanels.Add(subtitlePanelNumberFromPanel2);
				if (subtitlePanelNumberFromPanel2 < SubtitlePanelNumber.Panel0)
				{
					continue;
				}
				int index = (int)(subtitlePanelNumberFromPanel2 - 3);
				if (actorNames[index] == string.Empty)
				{
					actorNames[index] = item2.Key.name;
					continue;
				}
				DialogueActor dialogueActorComponent = DialogueActor.GetDialogueActorComponent(item2.Key);
				if (dialogueActorComponent != null && dialogueActorComponent.actor == value2.portraitActorName)
				{
					actorNames[index] = item2.Key.name;
				}
			}
		...
		public virtual void HideOnResponseMenu()
		{
			...
			{
				CloseSubtitlePanelOnResponseMenu(customPanel);
			}
		}

		protected virtual void CloseSubtitlePanelOnResponseMenu(StandardUISubtitlePanel panel)
		{
			if (panel != null && panel.isOpen && panel.visibility == UIVisibility.UntilSupercededOrActorChangeOrMenu)
			{
				panel.Close();
			}
		}
		...
		private void CheckActorIDOnStartConversation(int actorID, HashSet<int> checkedActorIDs, HashSet<StandardUISubtitlePanel> checkedPanels, StandardDialogueUI ui)
		{
			...
			if (!(standardUISubtitlePanel == null) && !checkedPanels.Contains(standardUISubtitlePanel))
			{
				...
				if (standardUISubtitlePanel.visibility == UIVisibility.AlwaysFromStart)
				{
					...
					string name = actor.Name;
					string localizedDisplayNameInDatabase = CharacterInfo.GetLocalizedDisplayNameInDatabase(name);
					standardUISubtitlePanel.OpenOnStartConversation(portraitSprite, name, localizedDisplayNameInDatabase, dialogueActor, dialogueUI);
		...
		public void OpenSubtitlePanelLikeStart(SubtitlePanelNumber subtitlePanelNumber)
		{
			...
			for (int i = 0; i < conversation.dialogueEntries.Count; i++)
			{
				...
				if (standardUISubtitlePanel == panelFromNumber)
				{
					...
					string name = actor.Name;
					string localizedDisplayNameInDatabase = CharacterInfo.GetLocalizedDisplayNameInDatabase(name);
					panelFromNumber.OpenOnStartConversation(portraitSprite, name, localizedDisplayNameInDatabase, dialogueActor, dialogueUI);
					break;
				}
			}
		}
