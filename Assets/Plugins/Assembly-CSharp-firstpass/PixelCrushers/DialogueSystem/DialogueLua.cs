...
namespace PixelCrushers.DialogueSystem
{
	public static class DialogueLua
	{
		...
		public static void SetVariable(string variable, object value)
		{
			...
			if (Lua.Environment.GetValue("Variable") is LuaTable luaTable)
			{
				string text = StringToTableIndex(variable);
				luaTable.SetNameValue(text, LuaInterpreterExtensions.ObjectToLuaValue(value));
				if (Assignment.MonitoredVariables.Contains(text))
				{
					Assignment.InvokeVariableChanged(text, value);
				}
			}
		}
