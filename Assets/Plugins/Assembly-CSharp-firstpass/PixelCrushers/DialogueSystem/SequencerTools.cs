...
namespace PixelCrushers.DialogueSystem
{
	public static class SequencerTools
	{
		...
		public static T GetParameterAs<T>(string[] parameters, int i, T defaultValue)
		{
			try
			{
				if (parameters != null && 0 <= i && i < parameters.Length)
				{
					...
					if ((typeof(T) == typeof(float) || typeof(T) == typeof(int)) && text.EndsWith("f", StringComparison.OrdinalIgnoreCase))
					{
		...
		public static AudioSource GetAudioSource(Transform subject)
		{
			...
			DialogueActor component = gameObject.GetComponent<DialogueActor>();
