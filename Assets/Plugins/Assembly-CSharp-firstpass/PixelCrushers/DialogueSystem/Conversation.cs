	...
	public class Conversation : Asset
	{
		...
		private void SplitEntryAtPipes(int originalEntryIndex, string dialogueText, bool putEndSequenceOnLastSplit, bool trimWhitespace, string uniqueFieldTitle = null)
		{
			...
			while (num < array.Length)
			{
				...
				if (string.IsNullOrEmpty(array[num]) && num == array.Length - 1)
				{
					num++;
					continue;
				}
				string text2 = ((num < array2.Length) ? array2[num] : string.Empty);
				if (trimWhitespace)
				{
					text = text.Trim();
					text2 = text2.Trim();
				}
				DialogueEntry dialogueEntry3 = AddNewDialogueEntry(dialogueEntry, text, num, trimWhitespace);
				dialogueEntry3.canvasRect = new Rect(dialogueEntry.canvasRect.x + (float)(num * 20), dialogueEntry.canvasRect.y + (float)(num * 10), dialogueEntry.canvasRect.width, dialogueEntry.canvasRect.height);
				dialogueEntry3.currentMenuText = text2;
				dialogueEntry3.AudioFiles = $"[{((num < array3.Length) ? array3[num] : string.Empty)}]";
				if (flag)
				{
					Field.SetValue(dialogueEntry3.fields, uniqueFieldTitle, $"{arg}-{num}");
				}
