...
namespace PixelCrushers.DialogueSystem
{
	...
	public class ExtraDatabases : MonoBehaviour
	{
		...
		protected DialogueDatabase[] databaseInstances;
		...
		{
			if (DialogueManager.instance == null || m_trying)
			{
				return;
			}
			m_trying = true;
		...
		public virtual void AddDatabases(bool onePerFrame)
		{
			if (DialogueManager.instance.instantiateDatabase)
			{
				...
				{
					DialogueDatabase[] array = databaseInstances;
				...
				if (databaseInstances == null)
				{
					...
					{
						databaseInstances[j] = UnityEngine.Object.Instantiate(databases[j]);
					}
				}
			}
			if (onePerFrame)
