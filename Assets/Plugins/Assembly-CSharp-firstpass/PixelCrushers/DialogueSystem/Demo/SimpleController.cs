...
namespace PixelCrushers.DialogueSystem.Demo
{
	...
	public class SimpleController : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			float num = Mouse.current.delta.x.ReadValue() * Time.deltaTime;
			float num2 = Mouse.current.delta.y.ReadValue() * Time.deltaTime;
			base.transform.Rotate(0f, num * mouseSensitivityX, 0f);
			m_cameraRotationY += num2 * mouseSensitivityY;
			...
			float axis = InputDeviceManager.GetAxis(verticalAxis);
			float axis2 = InputDeviceManager.GetAxis(horizontalAxis);
			...
			{
				m_controller.Move(base.transform.rotation * (Vector3.forward * axis * runSpeed * Time.deltaTime + Vector3.right * axis2 * runSpeed * Time.deltaTime) + Vector3.down * 20f * Time.deltaTime);
			}
		}
