			...
			for (int i = 0; i < standardDialogueUI2.conversationUIElements.subtitlePanels.Length; i++)
			{
				...
				if (0 <= i && i < data.panelOpenOnActorName.Count && !string.IsNullOrEmpty(data.panelOpenOnActorName[i]))
				{
					...
					else if (actor2 != null)
					{
						...
						text = actor2.Name;
						displayName = CharacterInfo.GetLocalizedDisplayNameInDatabase(text);
					}
					if (!standardUISubtitlePanel.isOpen)
					{
						standardUISubtitlePanel.OpenOnStartConversation(portraitSprite, text, displayName, dialogueActor);
					}
				}
