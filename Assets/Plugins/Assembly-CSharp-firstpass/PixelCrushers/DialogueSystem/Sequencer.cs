...
namespace PixelCrushers.DialogueSystem
{
	public class Sequencer : MonoBehaviour
	{
		...
		private void InformParticipants(string message)
		{
			if (m_speaker != null)
			{
				Transform parameter = ((m_listener != null) ? m_listener : m_speaker);
				m_speaker.BroadcastMessage(message, parameter, SendMessageOptions.DontRequireReceiver);
			...
			{
				Transform parameter2 = ((m_speaker != null) ? m_speaker : ((m_listener != null) ? m_listener : DialogueManager.instance.transform));
				DialogueManager.instance.BroadcastMessage(message, parameter2, SendMessageOptions.DontRequireReceiver);
			}
		}
		...
		private void CheckQueuedCommands()
		{
			...
			float time = DialogueTime.time;
			try
			{
				for (int num = m_queuedCommands.Count - 1; num >= 0; num--)
				{
					if (0 <= num && num < m_queuedCommands.Count)
					{
						QueuedSequencerCommand queuedSequencerCommand = m_queuedCommands[num];
						if (time >= queuedSequencerCommand.startTime)
						{
							m_queuedCommands.RemoveAt(num);
		...
		private void StopTimedSequencerMessageCoroutines()
		{
			...
			foreach (string queuedDelayMessage in queuedDelayMessages)
			{
				Message(queuedDelayMessage);
			}
		...
		public void StopActive()
		{
			foreach (SequencerCommand activeCommand in m_activeCommands)
			{
				if (activeCommand != null)
				{
					...
					{
						Message(activeCommand.endMessage);
					}
		...
		private bool HandleOpenPanelInternally(string commandName, string[] args)
		{
			...
			if (standardDialogueUI != null)
			{
				...
				if (0 <= subtitlePanelIndex && subtitlePanelIndex < subtitlePanels.Length)
				{
					...
					else if (string.Equals("portrait", text, StringComparison.OrdinalIgnoreCase))
					{
						Tools.SetGameObjectActive(standardUISubtitlePanel.portraitImage, value: true);
						Tools.SetGameObjectActive(standardUISubtitlePanel.portraitName.gameObject, value: true);
					}
					else if (string.Equals("portraitimage", text, StringComparison.OrdinalIgnoreCase))
					{
						Tools.SetGameObjectActive(standardUISubtitlePanel.portraitImage, value: true);
					}
					else if (DialogueDebug.logWarnings)
		...
		private bool HandleHidePanelInternally(string commandName, string[] args)
		{
			...
			bool flag3 = string.Equals("immediate", SequencerTools.GetParameter(args, 1), StringComparison.OrdinalIgnoreCase) || string.Equals("immediate", SequencerTools.GetParameter(args, 2), StringComparison.OrdinalIgnoreCase);
			StandardDialogueUI standardDialogueUI = DialogueManager.dialogueUI as StandardDialogueUI;
		...
		private bool HandleSetPortraitInternally(string commandName, string[] args)
		{
			...
			else
			{
				if (!flag)
				{
					...
					{
						if (asset is Sprite sprite2)
						{
							DialogueLua.SetActorField(actorName, "Current Portrait", textureName);
							DialogueManager.instance.SetActorPortraitSprite(actorName, sprite2);
						}
						else if (asset is Texture2D texture)
						{
							Sprite sprite3 = UITools.CreateSprite(texture);
							if (sprite3 == null && DialogueDebug.logWarnings)
							{
							...
							DialogueLua.SetActorField(actorName, "Current Portrait", textureName);
							DialogueManager.instance.SetActorPortraitSprite(actorName, sprite3);
						}
					});
					return true;
				}
